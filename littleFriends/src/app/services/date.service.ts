...
export class DateService {
    ...
    getMediumDateDisplay(dateValue: number): string {
        let myFormattedDate = pipe.transform(dateValue, 'mediumDate') as string;
        return myFormattedDate ;
    }

    getShortDateAndTimeDisplay(dateValue: number): string {
        let myFormattedDate = pipe.transform(dateValue, 'short') as string;
        return myFormattedDate;
    }

    getShortTimeDisplay(dateValue: number): string {
        let myFormattedDate = pipe.transform(dateValue, 'shortTime') as string;
        return myFormattedDate;
    }
